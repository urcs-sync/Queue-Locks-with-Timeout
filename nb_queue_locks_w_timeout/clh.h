...
typedef struct {
    clh_cc_qnode_ptr tail;
    clh_cc_qnode *lock_holder;   /* node allocated by lock holder,
...
extern bool clh_try_timed_acquire_slowpath(clh_cc_lock *L,
...
INLINE bool clh_try_timed_acquire(clh_cc_lock *L, clh_cc_qnode *I, hrtime_t T)
...
/******** blocking timeout, bounded space, swap only: ********/

#define CLH_TRY_SWAP_ACQUIRE_BODY \
    clh_cc_qnode *pred; \
    I->status = waiting; \
    pred = clh_qn_swap(L, I); \
    while (1) { \
...
INLINE void clh_try_swap_acquire(clh_cc_lock *L, clh_cc_qnode *I)
BODY({CLH_TRY_SWAP_ACQUIRE_BODY})

extern bool clh_try_swap_timed_acquire_slowpath(clh_cc_lock *L,
    clh_cc_qnode *I, clh_cc_qnode *pred, hrtime_t T);

#define CLH_TRY_SWAP_TIMED_ACQUIRE_BODY \
    clh_cc_qnode *pred; \
    I->status = waiting; \
    pred = clh_qn_swap(L, I); \
    if (pred->status == available) { \
        ...
        return clh_try_swap_timed_acquire_slowpath(L, I, pred, T); \
    }

INLINE bool clh_try_swap_timed_acquire(clh_cc_lock *L,
    clh_cc_qnode *I, hrtime_t T)
BODY({CLH_TRY_SWAP_TIMED_ACQUIRE_BODY})

#define CLH_TRY_SWAP_RELEASE_BODY \
    qnode_status stat; \
    clh_cc_qnode *pred = (*I)->prev; \
    /* can't set my node to available if it's currently transient */ \
    while (1) { \
        if ((stat = s_swap(&(*I)->status, transient)) == waiting) break; \
        while ((*I)->status == transient);  /* spin */ \
    } \
    (*I)->status = available; \
    *I = pred;

INLINE void clh_try_swap_release(clh_cc_qnode **I)
BODY({CLH_TRY_SWAP_RELEASE_BODY})

/******** non-blocking timeout, unbounded space, standard interface: ********/

extern void clh2_try_acquire(clh2_cc_lock *L);
extern bool clh2_try_timed_acquire(clh2_cc_lock *L, hrtime_t T);
extern void clh2_try_release(clh2_cc_lock *L);

#ifdef FUTURE_WORK

    /* non-cache-coherent machine: */
    void clh_try_numa_acquire(clh_numa_lock *L, clh_numa_qnode *I);
    bool clh_try_numa_timed_acquire(clh_numa_lock *L,
        clh_numa_qnode *I, hrtime_t T);
    void clh_try_numa_release(clh_numa_qnode **I);

    /* non-cache-coherent machine, swap only: */
    extern void clh_try_numa_swap_acquire(clh_cc_lock *L, clh_cc_qnode *I);
    extern bool clh_try_numa_swap_timed_acquire(clh_cc_lock *L,
        clh_cc_qnode *I, hrtime_t T);
    extern void clh_try_numa_swap_release(clh_cc_qnode **I);
